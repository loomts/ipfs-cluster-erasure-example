...
var add_same512MB = []float64{91.184931047, 96.774071187, 98.24160554, 95.668099063, 84.502639526, 93.280970291, 94.990654178, 94.637416535, 95.898682755, 97.059757424, 99.004907566, 100.243467094, 94.992492943, 97.66777779, 100.014414768, 98.809110187, 98.057695412, 102.907488774, 103.413974143, 101.937286929, 98.571185264}
var ecadd_same512MB = []float64{66.538174167, 64.226807128, 54.910991772, 57.823912333, 55.398362483, 62.99811361, 56.507358633, 55.489491385, 58.201098864, 57.587954491, 58.101931719, 54.921586059, 67.752596118, 66.998794989, 56.746959761, 66.741377911, 66.119332201, 73.229681408, 59.574420691, 58.431667209, 56.397626786000004}
...
func Cal() {
	calsame := func(times []float64, name string) {
		...
		avg := float64(len(ecadd_same512MB)*512) / total
	...
	calsame(ecadd_same512MB, "add --erasure same512")
	calsame(add_same512MB, "add --erasure same512")
...
func DrawAddSame() {
	...
	p.Y.Max = 105
	plotutil.AddLinePoints(p, "add", pointsFromXYs(sameSize, add_same512MB), "add --erasure", pointsFromXYs(sameSize, ecadd_same512MB))
